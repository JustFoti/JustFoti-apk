  ...
  return (
    <section className="py-4 md:py-6 px-3 md:px-6">
      <div className="container mx-auto">
        ...
        <div ref={scrollRef} className="flex gap-2.5 md:gap-4 overflow-x-auto scrollbar-hide pb-3 md:pb-4 -mx-1 px-1 md:-mx-2 md:px-2 snap-x snap-mandatory" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none', WebkitOverflowScrolling: 'touch' }} data-tv-scroll-container="true" data-tv-group={`movies-${title.toLowerCase().replace(/[^a-z]/g, '')}`}>
          {data.items.map((item, index) => (
            <motion.div key={item.id} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: '-50px' }} transition={{ delay: Math.min(index * 0.03, 0.3) }} onClick={() => onItemClick(item, title)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onItemClick(item, title); } }} className="flex-shrink-0 w-28 sm:w-32 md:w-36 lg:w-44 cursor-pointer group snap-start" data-tv-focusable="true" tabIndex={0} role="button" aria-label={item.title || item.name || ''}>
