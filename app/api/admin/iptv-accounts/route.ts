/**
 * IPTV Accounts Management API
 * 
 * Manages IPTV portal accounts for live TV streaming.
 * Uses D1 database after Cloudflare migration.
 */

import { NextRequest, NextResponse } from 'next/server';
import { verifyAdminAuth } from '@/app/lib/utils/admin-auth';
import { getD1Database } from '@/app/lib/db/d1-connection';
import { v4 as uuidv4 } from 'uuid';

// Initialize IPTV tables if they don't exist
async function ensureIPTVTables() {
  const db = getD1Database();
  
  await db.prepare(`
    CREATE TABLE IF NOT EXISTS iptv_accounts (
      id TEXT PRIMARY KEY,
      portal_url TEXT NOT NULL,
      mac_address TEXT NOT NULL,
      name TEXT,
      channels_count INTEGER DEFAULT 0,
      stream_limit INTEGER DEFAULT 1,
      active_streams INTEGER DEFAULT 0,
      status TEXT DEFAULT 'active',
      last_tested INTEGER,
      last_used INTEGER,
      total_usage_count INTEGER DEFAULT 0,
      error_count INTEGER DEFAULT 0,
      last_error TEXT,
      priority INTEGER DEFAULT 0,
      created_at INTEGER DEFAULT(strftime('%s', 'now') * 1000),
      updated_at INTEGER DEFAULT(strftime('%s', 'now') * 1000),
      UNIQUE(portal_url, mac_address)
    )
  `).run();
  
  // Create indexes
  await db.prepare('CREATE INDEX IF NOT EXISTS idx_iptv_accounts_status ON iptv_accounts(status)').run();
  await db.prepare('CREATE INDEX IF NOT EXISTS idx_iptv_accounts_priority ON iptv_accounts(priority DESC)').run();
}

export async function GET(request: NextRequest) {
  const authResult = await verifyAdminAuth(request);
  if (!authResult.success) {
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  }

  try {
    await ensureIPTVTables();
    const db = getD1Database();
    
    const result = await db.prepare(`
      SELECT * FROM iptv_accounts 
      ORDER BY priority DESC, channels_count DESC, created_at DESC
    `).all();
    
    return NextResponse.json({ 
      success: true, 
      accounts: result.results,
      total: result.results.length 
    });
  } catch (error: any) {
    console.error('Failed to fetch IPTV accounts:', error);
    return NextResponse.json({ 
      success: false, 
      error: error.message 
    }, { status: 500 });
  }
}

export async function POST(request: NextRequest) {
  const authResult = await verifyAdminAuth(request);
  if (!authResult.success) {
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  }

  try {
    await ensureIPTVTables();
    const db = getD1Database();
    const body = await request.json();
    const { action } = body;

    switch (action) {
      case 'add': {
        const { portal_url, mac_address, name, channels_count, stream_limit, priority } = body;
        
        if (!portal_url || !mac_address) {
          return NextResponse.json({ error: 'Portal URL and MAC address required' }, { status: 400 });
        }

        const id = uuidv4();
        const now = Date.now();

        await db.prepare(`
          INSERT OR REPLACE INTO iptv_accounts (id, portal_url, mac_address, name, channels_count, stream_limit, priority, created_at, updated_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).bind(id, portal_url, mac_address, name || null, channels_count || 0, stream_limit || 1, priority || 0, now, now).run();

        return NextResponse.json({ success: true, id });
      }

      case 'import': {
        const { accounts } = body;
        
        if (!Array.isArray(accounts) || accounts.length === 0) {
          return NextResponse.json({ error: 'No accounts to import' }, { status: 400 });
        }

        const now = Date.now();
        let imported = 0;

        for (const acc of accounts) {
          if (!acc.portal_url || !acc.mac_address) continue;
          
          const id = uuidv4();
          try {
            await db.prepare(`
              INSERT OR IGNORE INTO iptv_accounts (id, portal_url, mac_address, name, channels_count, stream_limit, priority, created_at, updated_at)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            `).bind(id, acc.portal_url, acc.mac_address, acc.name || null, acc.channels_count || 0, acc.stream_limit || 1, acc.priority || 0, now, now).run();
            imported++;
          } catch (e) {
            // Skip duplicates
          }
        }

        return NextResponse.json({ success: true, imported });
      }

      case 'update': {
        const { id, ...updates } = body;
        
        if (!id) {
          return NextResponse.json({ error: 'Account ID required' }, { status: 400 });
        }

        const allowedFields = ['name', 'stream_limit', 'priority', 'status', 'channels_count'];
        const setClause: string[] = [];
        const values: any[] = [];
        
        for (const [key, value] of Object.entries(updates)) {
          if (allowedFields.includes(key)) {
            setClause.push(`${key} = ?`);
            values.push(value);
          }
        }

        if (setClause.length === 0) {
          return NextResponse.json({ error: 'No valid fields to update' }, { status: 400 });
        }

        setClause.push('updated_at = ?');
        values.push(Date.now());
        values.push(id);

        await db.prepare(
          `UPDATE iptv_accounts SET ${setClause.join(', ')} WHERE id = ?`
        ).bind(...values).run();

        return NextResponse.json({ success: true });
      }

      case 'delete': {
        const { id } = body;
        
        if (!id) {
          return NextResponse.json({ error: 'Account ID required' }, { status: 400 });
        }

        await db.prepare('DELETE FROM iptv_accounts WHERE id = ?').bind(id).run();

        return NextResponse.json({ success: true });
      }

      case 'deleteAll': {
        // Delete all accounts
        await db.prepare('DELETE FROM iptv_accounts').run();
        // Also delete all channel mappings since they reference accounts
        try {
          await db.prepare('DELETE FROM channel_mappings').run();
        } catch {
          // Table might not exist
        }
        return NextResponse.json({ success: true });
      }

      case 'test': {
        // Test connection to a specific account
        const { id, portal_url, mac_address } = body;
        
        // Use the existing IPTV debug endpoint logic
        const testResponse = await fetch(`${request.nextUrl.origin}/api/admin/iptv-debug`, {
          method: 'POST',
          headers: { 
            'Content-Type': 'application/json',
            'Cookie': request.headers.get('cookie') || '',
          },
          body: JSON.stringify({ action: 'test', portalUrl: portal_url, macAddress: mac_address })
        });
        
        const testResult = await testResponse.json();
        const now = Date.now();
        
        // Update account status based on test result
        if (id) {
          if (testResult.success) {
            await db.prepare(`
              UPDATE iptv_accounts SET 
                status = 'active', 
                channels_count = ?, 
                last_tested = ?, 
                last_error = NULL,
                error_count = 0,
                updated_at = ?
              WHERE id = ?
            `).bind(testResult.content?.itv || 0, now, now, id).run();
          } else {
            await db.prepare(`
              UPDATE iptv_accounts SET 
                status = 'error', 
                last_tested = ?, 
                last_error = ?,
                error_count = error_count + 1,
                updated_at = ?
              WHERE id = ?
            `).bind(now, testResult.error || 'Connection failed', now, id).run();
          }
        }

        return NextResponse.json({ 
          success: testResult.success, 
          channels: testResult.content?.itv || 0,
          error: testResult.error 
        });
      }

      default:
        return NextResponse.json({ error: 'Invalid action' }, { status: 400 });
    }
  } catch (error: any) {
    console.error('IPTV accounts error:', error);
    return NextResponse.json({ 
      success: false, 
      error: error.message 
    }, { status: 500 });
  }
}
