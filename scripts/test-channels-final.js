...
const TEST_CHANNELS = [
  ...
  { id: '38', name: 'Sky Sports Main Event [UK]' },
  { id: '44', name: 'ESPN [USA]' },
  { id: '51', name: 'ABC [USA]' },
...
async function testChannel(channel) {
  ...
  try {
    ...
    return { ...channel, status: 'PARTIAL', error: keyText.substring(0, 60), time: Date.now() - start };
  } catch (e) {
...
async function main() {
  console.log('=== Final Channel Test ===\n');
  
  let ok = 0, partial = 0, fail = 0;
  
  for (const channel of TEST_CHANNELS) {
    process.stdout.write(`${channel.id.padStart(3)} ${channel.name.padEnd(28)} `);
  ...
  console.log(`\n=== Results: ${ok} OK, ${partial} Partial, ${fail} Failed ===`);
}
