...
async function main() {
  ...
  let page = 0;
  while (page < 10) {
    const result = (await makeRequest(PORTAL_URL + `portal.php?type=itv&action=get_ordered_list&genre=671&p=${page}&JsHttpRequest=1-xml`, MAC, token)).js;
    if (!result.data?.length) break;
    
    const matches = result.data.filter(ch => ch.name.toUpperCase().includes('SPORTSNET'));
    if (matches.length) {
